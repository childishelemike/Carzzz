...
function drawBg() {
    // Create gradient
    const skyGradient = $.ctx.createLinearGradient(0, 0, 0, $.settings.skySize);

    // Add color stops
    skyGradient.addColorStop(0, "#FCB944");  // Top color
    skyGradient.addColorStop(1, "#FF6401");  // Bottom color (adjust as desired)
  
    // Apply gradient
    $.ctx.fillStyle = skyGradient;
    $.ctx.fillRect(0, 0, $.canvas.width,$.canvas.height, $.settings.skySize);
...
function drawCarBody(ctx) {
  ...
  /* Front */
  roundedRect($.ctx, "#D62828", startX + 6 + ($.state.turn * 1.1), startY - 18, 146, 40, 18);
  
  ctx.beginPath(); 
  ctx.lineWidth="12";
  ctx.fillStyle="#DF5353";
  ctx.strokeStyle="#EC9898";
  ...
  roundedRect(ctx, "#561010", startX - 4, startY, 169, 10, 3, true, 0.2);
  roundedRect(ctx, "#340909", startX + 40, startY + 5, 80, 10, 5, true, 0.1);
